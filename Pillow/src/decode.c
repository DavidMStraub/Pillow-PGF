...
PyImaging_PgfDecoderNew(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "s|i", &mode, &fd)) {
        return NULL;
    }

    decoder = PyImaging_DecoderNew(sizeof(PGFDECODESTATE));
    if (decoder == NULL) {
        return NULL;
    }

    decoder->pulls_fd = 1;
    decoder->decode = ImagingPgfDecode;
    decoder->cleanup = ImagingPgfDecodeCleanup;

    PGFDECODESTATE *context = (PGFDECODESTATE *)decoder->state.context;
